...
/*
Tabella che mostra come i caratteri ASCII vengono disegnati sullo schermo
...
/**
**L'impostazione predefinita SCREEN_WIDTH è 80.
**L'impostazione predefinita SCREEN_HEIGHT è 25.
**/
#ifndef SCREEN_WIDTH
...
enum FrameType : unsigned short int
{
    DOUBLE = 0,
    SINGLE = 1
};
...
protected:
    /**
     **Costruttore di giochi.
     **
     **I nomi sono autoesplicativi.
     **È importante impostare correttamente il buffer,
     **inizializzandolo con qualcosa.
     **/
    Game()
    {
     ...
     **
     **Riempi la cella selezionata (buffer[y][x]) con
     **un ASCII CHAR c e alcuni ATTRIBUTI attr.
     **/
     ...
     **DrawBuffer():
     **
     **Disegna il buffer nello schermo.
     **
     **Chiamalo dopo aver disegnato qualcosa.
     **/
    void DrawBuffer()
    {
    ...
    /**
     **DrawBufferRegion (SHORT x, SHORT y, colonne SHORT, righe SHORT):
     **
     **Disegna una regione del buffer sullo schermo.
     **La regione è definita dall'angolo in alto a sinistra
     **Coordinate [x, y] e numero di righe
     **e colonne.
     **
     **Chiamalo dopo aver disegnato qualcosa.
     **/
     ...
     **
     **Riempire l'intero buffer con un ASCII CHAR c
     **e alcuni ATTRIBUTI attr.
     **
     **es. Usa FillBuffer(' ', F_BLACK | B_BLACK)
     **per visualizzare lo schermo nero;
     **/
    ...
    /**
     **IsKeyDown (chiave int):
     **
     **Controlla se la chiave è giù.
     **/
    ...
    /**
     **IsKeyPressed (tasto int):
     **
     **Verifica che il tasto sia premuto da allora
     **l'ultima chiamata.
     **/
    ...
    /**
     **DrawFrame(int x, int y, int w, int h, tipo FrameType, unsigned char attr):
     **
     **Disegna una cornice semplice usando x e y
     **coordinate, w & h come larghezza e altezza
     **e attr per visualizzarlo colorato.
     **
     **es.
     **Tipo di telaio:: SINGOLO Tipo di telaio:: DOPPIO
     **		┌──┐ ╔══╗
     **		│  │ ║  ║
     **		└──┘ ╚══╝
     **/
     ...
     **
     **Disegna del testo sullo schermo usando
     **Coordinate x & y e valore attr
     **per visualizzarlo colorato.
     **
     **Il testo è un array const char e n lo è
     **		la sua taglia.
     **Passa NULL per disegnare l'intero testo.
     **/
    ...
    /**
     **SetWindowSize (larghezza CORTA, altezza CORTA, bool adjustBuffer):
     **
     **Imposta la dimensione della finestra e se lo è il flag adjustBuffer
     **impostato su true, anche il buffer viene ridimensionato.
     **
     **/
    void SetWindowSize(SHORT width, SHORT height, bool adjustBuffer)
    {
        ...
        if (adjustBuffer)
            SetConsoleScreenBufferSize(consoleOutput, {width, height});
        SetConsoleWindowInfo(consoleOutput, TRUE, &rect);
    }

    /**
     **ShowConsoleCursor(BOOL visibile):
     **
     ...
     **
     **Impostare la posizione del cursore su [x, y].
     **/
